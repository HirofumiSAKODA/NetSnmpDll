// SnmpApi.cpp: CSnmpApi クラスのインプリメンテーション
//
//////////////////////////////////////////////////////////////////////
#include <stdafx.h>
#include "SnmpApi.h"
...
oid *CSnmpApi::snmp_pars_oOid(char *argv,oid *root,size_t *rootlen)
{
...
BOOL CSnmpApi::GetReq(int nMode,CString szIP, CString szComOrUser,CString szPasswd, CString szVarList, int wTimeOut, int wRetry)
{
...
BOOL CSnmpApi::GetNextReq(int nMode,CString szIP, CString szComOrUser,CString szPasswd, CString szVarList, int wTimeOut, int wRetry)
{
	if( szIP.IsEmpty() ||	szVarList.IsEmpty() ) return(FALSE);
	m_szIP = szIP;
	m_wTimeOut = wTimeOut*1000;
...
BOOL CSnmpApi::Walk(int nMode,CString szIP, CString szComOrUser,CString szPasswd, CString szVarList,BOOL bBulkMode, int wTimeOut, int wRetry)
{
	if( szIP.IsEmpty() ||	szVarList.IsEmpty() ) return(FALSE);
	m_bMIBOver = FALSE;
...
char CSnmpApi::GetType(char *szName) 
{
	struct tree *tp;
	tp= get_tree_head();
...
BOOL CSnmpApi::SetReq(int nMode,CString szIP, CString szComOrUser,CString szPasswd, CString szVarList, int wTimeOut, int wRetry)
{
	if( szIP.IsEmpty() ||	szVarList.IsEmpty() ) return(FALSE);
	m_szIP = szIP;
	m_wTimeOut = wTimeOut*1000;
...
void CSnmpApi::SetVarList(CString & szVarList,CString szSep)
{
	CString szTmp;
	szTmp = szVarList;
	szTmp.TrimLeft();
...
int CSnmpApi::TrapRcv(int op,struct snmp_session *session,int reqid,struct snmp_pdu *pdu,void *magic)
{
	...
	if (op == NETSNMP_CALLBACK_OP_RECEIVED_MESSAGE){
		...
		sprintf_s(szAgentIP,sizeof(szAgentIP),"%d.%d.%d.%d"
			...
			*/
			...
			szTrapOid = CSnmpApi::GetShortName(szTrapOid);
			...
			for(vars = pdu->variables; vars; vars = vars->next_variable) {
				...
				ConvKanjiCode(vars,szTmp,sizeof(szTmp));
				szTrap +="\t  ";
				CString s = CSnmpApi::ConvShortNameVarBind(szTmp);
				if( s.Find("Hex-") !=-1 ){ 
					s.Replace("\n"," ");
					s.Replace("\r"," ");
					s.Replace("\t"," ");
...
BOOL CSnmpApi::StartTrapRcv(int wPort)
{
	char szIP[64];
	m_TrapList.RemoveAll();
...
BOOL CSnmpApi::SendTrap(CString szIP, CString szCom, CString szEID, CString szVarList, int wGen, int wSpe)
{
    struct snmp_session session;
    struct snmp_pdu *pdu;
    oid name[MAX_OID_LEN];
...
int CSnmpApi::CmpOidByName(CString szName1, CString szName2)
{
	char szName[1024];
...
// 
void CSnmpApi::InitSnmpApi(CString szType, CString szBaseDir, CString szLogFile)
{
	CString szTmpDir;
	CString szConfDir;
	CString szMibDir;
...
void CSnmpApi::LoadMIB(CString szLogFile,CString szMibDescr)
{
	shutdown_mib();
	if( !szLogFile.IsEmpty() ) {
...
void CALLBACK CSnmpApi::TimerCallBack(HWND hWnd,UINT uMsg,UINT_PTR idEvent,DWORD dwTime)
{
	CheckRcv();
}


void CSnmpApi::CloseSnmpApi(CString szType)
{
	::KillTimer(NULL,nIDTimer);
	snmp_shutdown(szType);
}

CString CSnmpApi::GetMibVal(CString &szIn, CString szMib)
{
	CString s1 = szMib+_T(" ");
	CString s2 = szMib+_T("=");
	int i = szIn.Find(s1);
...
int CSnmpApi::GetIntMibVal(CString &szVal)
{
	int i;
	CString s;
	s = szVal;
...
int CSnmpApi::GetIntMibVal(CString &szIn, CString szMib)
{
	CString szVal;
...
CString CSnmpApi::GetOid(char *pLabel)
{
    oid name[MAX_OID_LEN];
...
CString CSnmpApi::GetShortName(CString &szIn)
{
	CString szTmp;
	CString szLast;
	int i = szIn.Find("=");
...
double CSnmpApi::GetDoubleMibVal(CString &szVal)
{
	int i = szVal.Find("(");
...
double CSnmpApi::GetDoubleMibVal(CString &szIn, CString szMib)
{
	CString szVal;
	szVal = GetMibVal(szIn,szMib);
...
CString CSnmpApi::GetIPAddr(CString &szIPList, CString szIndex)
{
	CString s1;
	CString s2;
	CString szID;
...
int CSnmpApi::GetIndexList(CString &szIn, CString szMib, CStringList &slIndex)
{
	slIndex.RemoveAll();
	szMib.TrimLeft();
	szMib.TrimRight();
...
CString CSnmpApi::GetTypeName(int wType)
{
	CString cp;
	switch (wType) {
        case TYPE_TRAPTYPE:
...
CString CSnmpApi::ConvShortNameVarBind(CString szIn)
{
	CString szName;
	CString szVal;
	CString szRet;
...
CString CSnmpApi::GetTrapDescr(int wTrap)
{
    switch(wTrap){
...
CString CSnmpApi::GetSubTree(tree *tree)
{
    struct tree *tp;
	CString szRet;
	CString s;
...
CString CSnmpApi::GetVBStr(char *p,BOOL bOid)
{
	CString szRet;
	szRet.Empty();
...
void CSnmpApi::FindMibDescr(CString szPath)
{
	CFileFind ff;
	CString szFind;
...
void CSnmpApi::LoadMibDescr(CString szFile)
{
	CFile f;
...
void CSnmpApi::SetMibDescr(CString szNameIn,CString szDescr)
{
	char szName[1024];
    oid name[MAX_OID_LEN];
    size_t name_length;
...
CString CSnmpApi::GetGroupObject(CString szGroupName)
{
...
void  CSnmpApi::ConvKanjiCode(struct variable_list *vars,char *s,size_t nLenIn)
{
	char *pIn;
	char *pOut;
	char *pHex;
...
void CSnmpApi::GetSnmpV3Info(struct snmp_session * session)
{
...
int CSnmpApi::GetUsmUserList(CStringList &slUsmUserList)
{
...
BOOL CSnmpApi::AddUsmUser(CString szEngID,CString szUser,CString szAPass,CString szEPass)
{
	CString szLine;
	szLine.Empty();
	if( !szEngID.IsEmpty() ) {
...
BOOL CSnmpApi::DelUsmUser(CString szEngID,CString szUser)
{
...
CString    CSnmpApi::FormatEngID(u_char *pEngID,size_t nEngIDLen,BOOL bLong)
{
	CString szEngID;
	szEngID.Empty();
	int i;
...
CString    CSnmpApi::GetMySNMPv3Info(void)
{
	CString szRet;
	u_char szEngID[2560];
